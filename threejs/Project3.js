...
var axleModel;       // A model containing only an axle (thin cylinder with wheel on each end).
var carModel;        // A model containing only a car.
var diskworldModel;  // A model containing the diskworld.

var currentModel;   // Contains the visible objects in the scene, but
...
var carAxle1, carAxle2;  // The two axles in the carModel.
var diskworldAxle1, diskworldAxle2;  // The two axles in the car that is part of diskworld.
var growingTree;  // The tree in the center of diskworld that grows.
var carRotator;  // An object at the origin that has the diskworld car as its child;
...
function createWorld() {
	...
	viewpointLight = new THREE.DirectionalLight( 0xffffff, 0.3 );  // a light to shine in the direction the camera faces
	viewpointLight.position.set(0,0,1);  // shines down the z-axis
	scene.add(viewpointLight);
	sunLight = new THREE.HemisphereLight(0xffffff, 0xffffff, .7 );
	scene.add(sunLight);
...
function updateForFrame() {
	if (currentModel == axleModel) {
		...
		var treeScale = growingTree.scale.x;
		if (treeScale > 0) {
			treeScale -= 0.0005;
...
function doChangeModel() {
	...
	var newModel = axle ? axleModel : car ? carModel : diskworldModel;
...
function init() {
	...
	document.getElementById("World").onchange = doChangeModel;
	document.getElementById("viewLighting").onchange = e => {
		viewpointLight.intensity = e.target.checked ? 0.3 : 0;
		render();
	};
	document.getElementById("sunLight").onchange = e => {
		sunLight.intensity = e.target.value / 100.0
		render();
	};
	createWorld();
	render();
